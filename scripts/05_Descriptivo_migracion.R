...
Boxplots_migracion <- ggplot(df_long, aes(x = "", y = value)) +
  geom_boxplot(alpha = 0.5, color = "black", outlier.shape = NA, fill = "steelblue") +
  facet_wrap(~variable, scales = "free", ncol = 3) +
  labs(title = "Análisis de los patrones migratorios",
       x = "variables", y = "Valor del indicador")
Boxplots_migracion
ggsave("visualization/Bosplots_migracion.jpg", plot = Boxplots_migracion, width = 6, height =6, dpi = 300)
...
totalemigrants <- ggplot(df_migrants, aes(y = total_emigrants)) +
  geom_boxplot(fill = "steelblue", width = 0.5) +  
  ylab("Porcentaje") +
  ggtitle("Total emigrantes")  
totalemigrants
ggsave("visualization/Total emigrantes.jpg", plot = totalemigrants, width = 6, height =6, dpi = 300)
...
emigrantesmujeres <- ggplot(df_long_migrantes, aes(x = "", y = value)) +
...
ggsave("visualization/Mujeres emigrantes.jpg", plot = emigrantesmujeres, width = 10, height =6, dpi = 300)
