...
clustervisualizacion <- fviz_cluster(a,data=df_model)
clustervisualizacion
ggsave("visualization/Clusters.jpg", plot = clustervisualizacion, width = 10, height =6, dpi = 300)
...
clusterlogaritmico <- fviz_cluster(a,data=df_log)
clusterlogaritmico
ggsave("visualization/Clusters transformación logarítmica.jpg", plot = clusterlogaritmico, width = 10, height =6, dpi = 300)
...
# Crear el boxplot con dos grupos (hue) según el cluster
bosplotscluster <- ggplot(df_boxplot_long, aes(x = variable, y = valor, fill = factor(cluster))) + 
  geom_boxplot() +
  coord_flip()+
  labs(x = "Variable", y = "Valor", fill = "Cluster")
bosplotscluster
ggsave("visualization/Boxplots clusters.jpg", plot = bosplotscluster, width = 10, height =8, dpi = 300)
...
# Crear el boxplot con dos grupos (hue) según el cluster
clustergenero <- ggplot(df_final_genero, aes(x = variable, y = valor, fill = factor(cluster))) + 
  geom_boxplot() +
  coord_flip()+
  labs(x = "Variable", y = "Valor", fill = "Cluster")
clustergenero
ggsave("visualization/Boxplots clusters genero.jpg", plot = clustergenero, width = 10, height =8, dpi = 300)
...
mapaclusters <- ggplot(map_clusters) +
  geom_polygon(aes(x = long, y = lat, group = group, fill=cluster)) +
  coord_map(xlim=c(-180, 180)) +
  scale_fill_manual(values=cluster_colors) +
  theme_map()
mapaclusters
ggsave("visualization/Mapa clusters.jpg", plot = mapaclusters, width = 10, height =6, dpi = 300)
